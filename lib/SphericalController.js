...
var SphericalController = /** @class */ (function(_super) {
  ...
  SphericalController.prototype.getTweenPosition = function(
    ...
    var tween = Tween.get(this.pos).to(toObj, option.duration, option.easing);
  ...
  SphericalController.prototype.moveTarget = function(value, option) {
    option = EasingOption_1.EasingOption.init(option, this);
    var tween = Tween.get(this._cameraTarget.position).to(
  ...
  SphericalController.prototype.loop = function(type, min, max, option) {
    ...
    toObjMin[type] = toMin;
    var loop = function() {
      var tween = Tween.get(_this.pos, { loop: -1 })
        .to(toObjMax, option.duration, option.easing)
        .to(toObjMin, option.duration, option.easing);
      tween.addEventListener("change", _this.dispatchUpdateEvent);
  ...
  SphericalController.prototype.moveCameraShift = function(value, option) {
    option = EasingOption_1.EasingOption.init(option, this);
    var tween = Tween.get(this.cameraShift).to(
      { x: value.x, y: value.y, z: value.z },
      option.duration,
      option.easing
    );
